namespace HREngine.Bots
{
    internal class Settings
    {
        public void setSettings()
        {
			...
			Helpfunctions.Instance.ErrorLog("Loaded settings from cfg/settings.json");

            // play with these settings###################################
			this.enfacehp = silver.ConfigReader.cfg.enfacehp;  // hp of enemy when your hero is allowed to attack the enemy face with his weapon
			this.maxwide = silver.ConfigReader.cfg.maxwide;   // numer of boards which are taken to the next deep-lvl
			...
			this.playarround = silver.ConfigReader.cfg.playaroundaoe;  //play around some enemys aoe-spells?
            //these two probs are >= 0 and <= 100
			this.playaroundprob = silver.ConfigReader.cfg.playaroundprob;    //probability where the enemy plays the aoe-spell, but your minions will not die through it
			this.playaroundprob2 = silver.ConfigReader.cfg.playaroundprob2;   // probability where the enemy plays the aoe-spell, and your minions can die!

			this.enemyTurnMaxWide = silver.ConfigReader.cfg.enemyTurnMaxWide; // bords calculated in enemys-first-turn in first AI step (lower than enemySecondTurnMaxWide)
			this.enemyTurnMaxWideSecondTime = silver.ConfigReader.cfg.enemyTurnMaxWideSecondTime; // bords calculated in enemys-first-turn BUT in the second AI step (higher than enemyTurnMaxWide)
			this.enemySecondTurnMaxWide = silver.ConfigReader.cfg.enemySecondTurnMaxWide; // number of enemy-board calculated in enemys second TURN
			...
			this.useSecretsPlayArround = silver.ConfigReader.cfg.playaroundtraps; // playing arround enemys secrets

            this.alpha = 50; // weight of the second turn in calculation (0<= alpha <= 100)

			this.simulatePlacement = silver.ConfigReader.cfg.simulatePlacement;  // set this true, and ai will simulate all placements, whether you have a alpha/flametongue/argus
            //use this only with useExternalProcess = true !!!!

			this.useExternalProcess = silver.ConfigReader.cfg.simulatePlacement;; // use silver.exe for calculations a lot faster than turning it off (true = recomended)
