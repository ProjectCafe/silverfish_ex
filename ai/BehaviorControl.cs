namespace HREngine.Bots
{
    ...
    public class BehaviorControl : Behavior
    {
        ...
        public override float getPlayfieldValue(Playfield p)
        {
			...
			if (p.value > System.Int32.MinValue) return p.value;
            int retval = 0;
            int hpboarder = 10;
            if (p.ownHeroName == HeroEnum.warlock && p.enemyHeroName != HeroEnum.mage) hpboarder = 6;
			int aggroboarder = silver.ConfigReader.cfg.aggroboarder;
			...
			silver.ConfigReader.Read ("cfg/settings.json");
			int bigmobcount = silver.ConfigReader.cfg.bigMobAttack;
            ...
            foreach (Handmanager.Handcard hc in p.owncards)
            {
                ...
                {
                    mobsInHand++;
					if (hc.card.Attack >= bigmobcount) bigMobsInHand++;
                }
            }
